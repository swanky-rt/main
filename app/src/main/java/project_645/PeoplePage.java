...
public class PeoplePage implements Page{
    ...
    static final int ROW_SIZE = 115; //column 1 is 9 char, and column 2 is 30 char
    ...
    public PeoplePage(int pageId, File dataFile){
        this.pageId = pageId;
    ...
    public void deserializeRows() {
        for (int i = 0; i < getAllRows().length; ++i) {
            ...
            if (curRow != null) {
                String personId = new String(curRow.movieId, StandardCharsets.US_ASCII).replaceAll("\u0000", "");
                String name = new String(curRow.title, StandardCharsets.US_ASCII).replaceAll("\u0000", "");;
                deserializedRows[i][0] = personId;
                deserializedRows[i][1] = name;
            }
        }
    }
