...
public class QueryExecutor {
    ...
    String finalOutput = "output.csv";
    java.io.File file = new java.io.File(filePath + finalOutput);
    FileWriter outputFile = new FileWriter(file);

    public QueryExecutor() throws IOException {
    }

    // Returns the total number of I/Os made by the buffer manager
    public long executeQuery(String startRange, String endRange, int bufferSize, boolean usePrematerializedTempTable, boolean index) throws Exception {
        ...
        SelectionOperator testRangeSelection = null;
        if(index){
            MovieIndex movieIndex = new MovieIndex(bufferManager, File.MOVIE_TITLE_IDX, startRange, endRange);
            testRangeSelection = new SelectionOperator(movieIndex, ColumnNames.TITLE, startRange, endRange, bufferManager, true);
        }
        else{
...
//        // Step 8: join on
        ...
        Record curRecord;
        outputFile.write("title,name\n");
        while ((curRecord = bnlJoinOperator2.next()) != null) {
            outputFile.write(
                    curRecord.getTitleDeserialized() + "," +
                            curRecord.getName() + "\n");
        }
        while ((curRecord = bnlJoinOperator2.next()) != null) {
            System.out.println(curRecord.getTitleDeserialized() + "," + curRecord.getPersonIdDeserialized() + "," + curRecord.getMovieIdDeserialized());
