...
public class QueryExecutor {
    ...
    public void executeQuery(String startRange, String endRange, int bufferSize) throws IOException {
        // File paths (based on your structure and Utilities.java)
        String filePath = System.getProperty("user.dir") + "/main/app/src/main/java/project_645/DB files/"; // Correct path to your DB files
        ...
        TableScanOperator moviesScan = new TableScanOperator(bufferManager, File.MOVIES, new String[] {"movieId", "title"});
        Record result1;
        while ((result1 = moviesScan.next()) != null) {
            System.out.println(result1.getTitle() + "," + result1.getName());  // CSV format without spaces
        }
